...
class LocalUpdate(object):
    ...
    def update_weights(self, model, old_model, lr_c, lr_e, current_task, Waq, Wav):
        ...
        for iter in range(self.args.local_ep):
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                    ...
                    for name, param in model.named_parameters():
                        ...
                        if 'linear_a_v_{}'.format(current_task) in name:
                            for pre_param in Wav:
                                ort_loss += torch.abs(torch.mm(pre_param, param.T)).sum()
                    loss = loss_dce + 0.01 * l1_loss + 0.5 * ort_loss # + 5.0*loss_kd  + 0.05 * ort_loss
                ...
                for i, lbl in enumerate(labels):
                    lbl = lbl.item() 
                    label_feature_dict[lbl%20].append(feature[i].detach().cpu().numpy())
        ...
        # count_dict = {}
        # for i in range(class_mask[task_id][0], class_mask[task_id][-1]+1):
        #     count_dict[i] = 0.0
        # for item in label:
        #     count_dict[item] = label.count(item) / int(self.args.local_ep)
